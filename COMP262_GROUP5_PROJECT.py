...
review_frequency = df.groupby(['reviewerID', 'combined_text', 'reviewTime']).size().reset_index(name='review_count')
...
# Remove duplicate reviews based on reviewerID and cleaned review text
df.drop_duplicates(subset=['reviewerID', 'combined_text', 'reviewTime'], keep='first', inplace=True)
